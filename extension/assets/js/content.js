...
function ajax_request(payload) {
	return new Promise((resolve, reject) => {
		$.ajax({
			url: 'http://localhost/website1/extension/api/extension_db_manage.php',
			type: 'POST',
			data:payload,
	...
	function(request, sender, sendResponse) {
		if( request.message === "bookmark_add" ) {
			...
			$('#Modal_Bookmark select[name=bookmark_position]').on("change" ,function(e){
		  	...
		  	chrome.runtime.sendMessage({"message": "bookmark_added", "bookmark_name_added": bookmark_name_added});
			...
			var $form_fileupload = $('<form>', {id:'fileupload', action:'http://localhost/website1/extension/api/extension_db_manage.php', method: 'post', enctype:"multipart/form-data"});
